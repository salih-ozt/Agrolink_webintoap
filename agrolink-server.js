...
function containsSqlInjection(value) {
    ...
    const dangerousPatterns = [
        ...
        /\b(sleep|benchmark|waitfor|load_file|outfile|dumpfile)\s*\(/i
    ];
...
// Input değerini güvenli hale getir
function sanitizeSqlInput(value) {
    ...
    if (containsSqlInjection(value)) {
        throw new Error('Güvenlik ihlali: Zararlı SQL pattern tespit edildi');
    }
...
class SecureDatabase {
    ...
    async exec(sql) {
        // exec için SQL injection kontrolü (DDL için kullanılır)
        if (containsSqlInjection(sql)) {
            throw new Error('Güvenlik ihlali: exec içinde zararlı SQL tespit edildi');
        }
